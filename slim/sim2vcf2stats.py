...
# -*- coding: utf-8 -*-
"""
DominL Simulation Pipeline: SLiM → VCF → Summary Statistics

Workflow:
  1. Modify baseline SLiM script (Gravel_Eurasian_varyh.slim) with dominance, region, rep
  2. Run SLiM → tree sequence (.trees) + optional VCF
  3. Simplify tree, add neutral/synonymous mutations, write NS/noncoding/syn VCFs
  4. Combine VCFs, compute ancestry per window, extract population genetics stats
  5. Write per-window stats table → stat_1kseg_byexon/{segment}_{dominance}_rep{N}_1MB_stat.csv

After enough replicates across regions and dominance models, concatenate stats files
and feed into training/ (train_xgboost.py, train_compare_models.py).

Population labels: p1=archaic (Neanderthal), p2=African (YRI), p3=European (CEU)
"""
...
# Import path config; allow running from slim/ or pipeline root
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
from config.paths import DIR_MASTER, PATH_SLIM, SEGMENTS_DIR, STATS_OUTPUT_DIR

parser = argparse.ArgumentParser(
    description="Run SLiM simulations and compute summary statistics in genomic windows."
)
parser.add_argument('-s', '--sim', action="store", dest="sim_id",
                    help="simulation batch ID (default: 1)", default=1, type=int)
parser.add_argument('-g', '--gene', action="store", dest="gene_id",
                    help="genomic region index 1-N (default: 1)", default=1, type=int)
parser.add_argument('-d', '--dominance', action="store", dest="dominance_id",
                    help="dominance: 1=additive, 2=partial, 3=recessive (default: 1)",
                    default=1, type=int)
parser.add_argument('--dir-master', dest="dir_master", default=None,
                    help=f"simulation root (default: {DIR_MASTER})")
parser.add_argument('--path-slim', dest="path_slim", default=None,
                    help=f"path to SLiM executable (default: {PATH_SLIM})")
args = parser.parse_args()

batch = str(args.sim_id)
whichgene = int(args.gene_id)
whichh = int(args.dominance_id)
DIR_master = os.path.normpath(args.dir_master or DIR_MASTER).replace('\\', '/')
if not DIR_master.endswith('/'):
    DIR_master += '/'
Path_slim = args.path_slim or PATH_SLIM 
...
def update_par_file(temp_par, new_par, whichrep, region_name, dominance, dir_master):
    """Modify SLiM template with dominance, region, rep. dominance: 1=additive, 2=partial, 3=recessive."""
    dominance_line = 6
    m1_line = 11
    segment_line = 22
    outputline = 162

    with open(temp_par) as oldfile:
    ...
    for line_counter, line in enumerate(lines):
        ...
        if line_counter == dominance_line:
            ...
            fields[1] = str(0.0) + ',' if dominance == 3 else str(0.5) + ','
        elif line_counter == segment_line:
            fields[2] = 'readFile("' + dir_master + SEGMENTS_DIR + '/sim_seq_info_' + str(region_name) + '.txt");'
...
def vcf2genos(vcfpath, p1size, p2size, p3size):
    """Parse VCF into genotype arrays. p1=archaic (Neanderthal), p2=YRI, p3=CEU."""    
    vcf = open(vcfpath)
    start = 0
    startg = 0
    pos = []
...
if __name__ == "__main__":
    ...
    slim_template = os.path.join(script_dir, "Gravel_Eurasian_varyh.slim")

    os.chdir(DIR_master)

    segs_path = os.path.join(DIR_master, SEGMENTS_DIR)
    allfiles = [f for f in os.listdir(segs_path) if f.endswith("txt")]
    genes = [f.partition("info_")[2].partition(".txt")[0] for f in allfiles]
    ...
    while rep < num_rep:
        ...
        adm_time = 8734  # generation of admixture in SLiM script

        slim_rep = "rep" + str(rep) + "_model" + str(whichh) + thisgene + "Gravel_Eurasian_varyh.slim"
        slim_rep_out = slim_rep + ".output"
        update_par_file(slim_template, slim_rep, rep, thisgene, whichh, DIR_master)
        os.system('%s %s > %s' % (Path_slim, slim_rep, slim_rep_out))
        print("slim done")

        # === 2. Tree → VCF (NS, noncoding, syn) ===
        ...
        # Sample sizes: [YRI, CEU, CHB]; Neanderthal fixed at 2 in SLiM
        sampsize = [30, 30, 30]

        # Get exon definitions from segment info file
        exonfilepath = os.path.join(segs_path, thisfile)
        ...
        p3anc_window = calc_ancestry_window(anc_p3, starts, ends, len_genome, window_size)

        # === 4. Population genetics stats per window (p1=archaic, p2=YRI, p3=CEU) ===
        ...
        stats_out_dir = os.path.join(DIR_master, STATS_OUTPUT_DIR)
        os.makedirs(stats_out_dir, exist_ok=True)
        stats_table.to_csv(os.path.join(stats_out_dir, filename + "_1MB_stat.csv"), index=False)
